...
class TikaExtractor extends TextExtractorPluginBase {
  ...
  public function extract(File $file) {
    ...
    $cmd = $java . $param . ' -jar ' . escapeshellarg($tika);
    if (!empty($this->configuration['tika_config_path'])) {
      $tika_config = realpath($this->configuration['tika_config_path']);
      if ($tika_config) {
        $cmd .= ' --config=' . escapeshellarg($tika_config);
      }
    }
    $cmd .= ' -t ' . escapeshellarg($filepath);
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['tika_config_path'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Path to Tika configuration file'),
      '#description' => $this->t('Enter the full path to a Tika configuration XML file. This can be used to disable OCR or customize Tika behavior. Example: "/etc/tika/tika-config.xml". Leave empty to use Tika defaults.'),
  ...
  public function validateConfigurationForm(array &$form, FormStateInterface $form_state) {
    ...
    // Check Tika config path if provided.
    $tika_config_path = $values['tika_config_path'] ?? '';
    if (!empty($tika_config_path) && !file_exists($tika_config_path)) {
      $form_state->setError($form['text_extractor_config']['tika_config_path'], $this->t('Invalid path %path for Tika configuration file.', ['%path' => $tika_config_path]));
    }
  ...
  public function submitConfigurationForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->configuration['tika_config_path'] = $form_state->getValue(['text_extractor_config', 'tika_config_path']);
